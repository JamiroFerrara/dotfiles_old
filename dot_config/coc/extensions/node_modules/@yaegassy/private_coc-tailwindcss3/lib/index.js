...
async function activate(context) {
  if (!getConfigTailwindCssEnable())
    return;
  context.subscriptions.push(import_coc2.commands.registerCommand("tailwindCSS.forceActivate", async () => {
    import_coc2.window.showMessage(`Force activation of coc-tailwindcss3`);
  }));
  ...
  async function didOpenTextDocument(document) {
    ...
    const includeLanguages = import_coc2.workspace.getConfiguration("tailwindCSS").get("includeLanguages");
    if (includeLanguages)
    ...
    folder = getOuterMostWorkspaceFolder(folder);
    try {
      const configFiles = await (0, import_fast_glob.default)(import_path.default.join(import_coc2.Uri.parse(folder.uri).fsPath, "**/" + CONFIG_FILE_GLOB), {
